...
<script>
    ...
    if ($category == \App\Model\Article::CATEGORY_PRODUCT) {
        ...
        $banner_name = '';
        $ext_list_css = 'news pc_news';
        $ext_list_css_mobile = 'news m_news';
    }
    ?>
</script>
    ...
    <main class="page_main">
        ...
        <section class="page_con">
            <div class="layout">
                ...
                <ul class="scms_list {{ $ext_list_css }}">
                </ul>
                <div class="fenye">
                    <ul> </ul>
                    <select class="">
                ...
                <ul class="pro_hid"> </ul>
                <ul class="m_scms_list {{ $ext_list_css_mobile }}"> 数据加载中，请稍后... </ul>
                ...
                <script>
                    ...
                    function fetchArticleList() {
                        return $.ajax({
                            ...
                            if (resp.errcode === 0) {
                                ...
                                $('.scms_list').empty();
                                $('.scms_list').append(articleElementList)
                    ...
                    function mobileItemTemplate(article) {
                        @switch($category)
                        @case(\App\ Model\ Article::CATEGORY_PRODUCT)
                        ...
                        @break;
                        @case(\App\ Model\ Article::CATEGORY_APP)
                        return '<li>' +
                            '<a href="' + article.covers[0].img + '" data-caption="' + article.title + '">' +
                            '<div class="pic"><img src="' + article.covers[0].img + '" alt="' + article.title + '"></div>' +
                            '<div class="words"> <div class="box"> ' +
                            '<h5>' + article.title + '</h5>' +
                            '<i class="effect_ico"></i>' +
                            '</div></div>'
                        '</a></li>'
                        @break;
                        @case(\App\ Model\ Article::CATEGORY_NEWS)
                        return '<li class="img_scale">' +
                            '<a href="' + article.url + '">' +
                            '<div class="pic"><img src="' + article.covers[0].img + '" alt="' + article.title + '"></div>' +
                            '<div class="words">' +
                            '<h5>' + article.title + '</h5>' +
                            '<p>' + article.summary + '</p>' +
                            '<div class="bot"> <time>' + article.updated_at + '</time>' +
                    ...
                    var loading = {
                        ...
                        init: function() {
                            ...
                            for (var n = 0; n < loading._default; n++) {
                                lis.eq(n).appendTo(".m_scms");
                            }
                            $(".m_scms img").each(function() {
                        ...
                        loadMore: function() {
                            ...
                            fetchProductList().done(function() {
                                for (var i = 0; i < loading._loading; i++) {
                                    ...
                                    $(".m_scms").append(target);
                                    $(".m_scms img").eq(mLis + i).each(function() {
                                        $(this).attr('src', $(this).attr('realSrc'));
                                    });
                                }
                            });
